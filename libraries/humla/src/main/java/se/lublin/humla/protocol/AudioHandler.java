...
public class AudioHandler extends HumlaNetworkListener implements AudioInput.AudioInputListener {
    ...
    private volatile boolean mForceTransmit;
    ...
    public AudioHandler(Context context, HumlaLogger logger, int audioStream, int audioSource,
        ...
        if (!echoCancellationMethod.equals("none")) {
            // Keep Android routing in communication mode while the native Oboe/WebRTC pipeline owns capture processing.
    ...
    public void onAudioInputReceived(short[] frame, int frameSize) {
        boolean talking = mForceTransmit || mInputMode.shouldTransmit(frame, frameSize);
        talking &= !mMuted;
        mEncodeListener.onAudioInputLevel(calculateInputLevel(frame, frameSize));
        ...
        if (talking) {
            ...
            if (transmitAmplitudeBoost != 1.0f) {
                for (int i = 0; i < frameSize; i++) {
                    ...
                    float val = frame[i] * transmitAmplitudeBoost;
    ...
    private float calculateInputLevel(short[] frame, int frameSize) {
        if (frameSize <= 0) {
            return 0.0f;
        }
        long sum = 0;
        for (int i = 0; i < frameSize; i++) {
            sum += Math.abs((int) frame[i]);
        }
        float level = sum / (float) frameSize / Short.MAX_VALUE;
        return Math.min(1.0f, level * 8.0f);
    }

    public void setForceTransmit(boolean forceTransmit) {
        mForceTransmit = forceTransmit;
    }

    public void setAllowedVoiceSession(Integer session) {
        synchronized (mOutput) {
    ...
    public interface AudioEncodeListener {
        ...
        void onAudioInputLevel(float level);
    }
