...
public class HumlaService extends Service implements IHumlaService, IHumlaSession, HumlaConnection.HumlaConnectionListener, HumlaLogger, BluetoothScoReceiver.Listener {
    ...
    private Integer mAllowedVoiceSession;
    ...
    private volatile float mAudioInputLevel;
    ...
    private final AudioHandler.AudioEncodeListener mAudioInputListener =
            new AudioHandler.AudioEncodeListener() {
                ...
                @Override
                public void onAudioInputLevel(float level) {
                    mAudioInputLevel = level;
                }
            };
...
//            auth.addCeltVersions(Constants.CELT_11_VERSION);
    ...
    public void onConnectionSynchronized() {
        ...
        try {
            mAudioHandler = mAudioBuilder.initialize(
                    ...
                    mVoiceTargetId);
            mAudioHandler.setAllowedVoiceSession(mAllowedVoiceSession);
    ...
    public void onConnectionDisconnected(HumlaException e) {
        ...
        final AudioHandler audioHandler = mAudioHandler;
        if (audioHandler != null) {
            Thread audioShutdownThread = new Thread(new Runnable() {
                @Override
                public void run() {
                    audioHandler.shutdown();
                }
            }, "HumlaAudioShutdown");
            audioShutdownThread.start();
        }
    ...
    @Override
    public void setUserTexture(int session, byte[] texture) {
        Mumble.UserState.Builder usb = Mumble.UserState.newBuilder();
        usb.setSession(session);
        usb.setTexture(ByteString.copyFrom(texture));
        getConnection().sendTCPMessage(usb.build(), HumlaTCPMessageType.UserState);
    }
    ...
    public void setAllowedVoiceSession(Integer session) {
        mAllowedVoiceSession = session;
        if (mAudioHandler != null) {
            mAudioHandler.setAllowedVoiceSession(session);
        }
    }

    public void setForceTransmit(boolean forceTransmit) {
        if (mAudioHandler != null) {
            mAudioHandler.setForceTransmit(forceTransmit);
        }
    }

    public float getAudioInputLevel() {
        return mAudioInputLevel;
    }
