...
public class AudioOutput implements Runnable, AudioOutputSpeech.TalkStateListener {
    ...
    private volatile Integer mAllowedVoiceSession;
    ...
    public void setAllowedVoiceSession(Integer session) {
        ...
        try {
            if (session != null) {
                Iterator<Map.Entry<Integer, AudioOutputSpeech>> iterator = mAudioOutputs.entrySet().iterator();
                while (iterator.hasNext()) {
                    Map.Entry<Integer, AudioOutputSpeech> entry = iterator.next();
                    AudioOutputSpeech speech = entry.getValue();
                    if (speech.getSession() != session) {
                        speech.destroy();
                        iterator.remove();
                    }
                }
    ...
    public void run() {
        ...
        while(mRunning) {
            if(fetchAudio(mix, 0, mBufferSize)) {
                NativeAudioInput.nativeQueueEchoReference(mix, 0, mBufferSize);
    ...
    public void queueVoiceData(byte[] data, HumlaUDPMessageType messageType) {
        ...
        Integer allowedVoiceSession = mAllowedVoiceSession;
        if (allowedVoiceSession != null && session != allowedVoiceSession) {
            return;
        }
